...
import {
  ...
  Circle,
  useMap,
} from "react-leaflet"; // Import useMap hook
...
const Map = ({
  ...
  return (
    ...
    >
      ...
      />
      {radius &&
        circleCenter && ( // Changed from 'center' to 'circleCenter' here as well, if circle is tied to that state
          <>
            <Circle
              center={circleCenter}
      ...
      {displayItems.map((item) => {
        const coords =
          item?.address?.location?.coordinates || item?.location?.coordinates;
        const position =
        ...
        return (
          <Marker
            key={
              item._id ||
              `${item.title}-${item.description}-${item.location?.coordinates[0]}-${item.location?.coordinates[1]}`
            }
            position={position}
            icon={isSelected ? HighlightIcon : DefaultIcon}
            eventHandlers={{
              click: () =>
                !isDefaultMarker && onItemSelect && onItemSelect(item),
            }}
          >
            <Popup>
              {!isDefaultMarker ? (
